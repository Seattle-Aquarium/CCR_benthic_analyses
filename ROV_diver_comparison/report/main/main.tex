...
as the trigger of kelp loss than as the mechanism that maintains a deforested
state once canopy has been lost, by grazing on new recruits
...
There were a variety of challenges associated with the San Juan Islands sites,
...
imported into Adobe Lightroom Classic, cropped to remove lens distortion at the 
...
\end{settingsbox}
\caption{
The camera settings used by the GoPro HERO 12 camera with GoPro Labs firmware
installed on the camera's SD card.
These settings enabled 27.3 megapixel raw (GPR) and preview (JPG) photographs to
be taken every three seconds.
The GoPro cameras were also time synchronized before each flight via:
\url{https://gopro.github.io/labs/control/precisiontime/} enabling alignment
