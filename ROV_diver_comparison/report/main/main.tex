...
\newcommand{\volunteeracknowledgement}{%
	We want to thank our Seattle Aquarium AI Teachers for their tireless
	...
	Likewise, we would like to thank the 4,111 volunteers that have
	...
	Tolimieri, Ken Collins, Jackie Selbitschka, and Jan Friewald for their 
	feedback which greatly improved this report.
}
%% ====================================================================
%% DOCUMENT
%% ====================================================================
...
We evaluated agreement between platforms in two complementary ways: in level,
using negative binomial and beta-binomial generalized linear mixed models to
test whether one platform records more or less than the other, and in
pattern, using transect-level correlations to test whether the two platforms
rise and fall together across transects.
Our central finding is that the ROV and divers characterize coastal vegetation
similarly.
Red and green algal percent-cover did not differ significantly in level
between platforms, and both were strongly correlated across transects (red
algae $r=0.79$, green algae $r=0.93$).
Sugar kelp (\textit{Saccharina latissima}) and sieve kelp (\textit{Agarum
clathratum}), which divers count as individuals and the ROV records as
percent-cover, were likewise strongly correlated ($r=0.96$ and $r=0.88$,
respectively).
Critically, this agreement held after between-site variation was removed
(within-site \textit{Saccharina} $r=0.95$, \textit{Agarum} $r=0.82$, green
algae $r=0.92$, red algae $r=0.76$), demonstrating that the two platforms
resolve the same transect-scale pattern in vegetation, and not merely the
same broad contrast between sites.
Among macroinvertebrates, none of the ten overlapping taxa differed
significantly in level between platforms, while pattern agreement ranged from
strong and persisting within site for some taxa (e.g., burrowing sea cucumber,
ochre/mottled star) to absent for others (e.g., kelp crab, large anemone).
...
substantially narrowed this gap, and most of the pattern agreement we observed
for substrate operated between sites rather than among transects within a
site.
...
diver-based kelp forest monitoring --- particularly for tracking coastal
vegetation --- and we distill practical lessons for future
...
Distinguishing such local declines from broader regional variability is itself
an argument for repeatable, spatially extensive benthic survey coverage.
...
Sea urchin overgrazing does not appear to have initiated decline within Puget
Sound; elsewhere along the Pacific coast, urchins are generally understood less
as the trigger of kelp loss and more as the mechanism that maintains a 
deforested state once canopy has been lost, by grazing on new recruits
\citep{FilbeeDexter:2014aa}.
...
in Washington's kelp forests \citep{Randell:2026aa}.
...
post-processing burden: a dive team can finalize its data easily after a dive, 
whereas a comparable volume of imagery has traditionally taken many hours to 
annotate by hand.
...
At each site, we surveyed replicate $30~m$ transects at two depths using 
...
Our second objective is to distill lessons learned from this comparison so as to 
...
Department of Fish and Wildlife-Department of Natural Resources Habitat 
Strategic Initiative Lead using EPA Puget Sound Geographic Program funds, 
integrating ROV benthic methods testing and post-processing workflow 
...
These paired ROV-diver surveys took place at two newly established Aquarium 
Reef Check sites in Elliott Bay, with each site sampled at a ``deep'' ($10~m$) 
and ``shallow'' ($5~m$) zone.
Both sites were sampled in the summer (August--September) and winter 
...
the same taxa found at Centennial Park, as well as burrowing sea cucumbers 
...
their depth structure, and season to focus on the focal questions 
investigated and answered in this technical report: 
(\textbf{1}) 
Do the data differ significantly by ``type,'' i.e., diver versus ROV?
(\textbf{2})
Do the two platforms agree in pattern, i.e., do they rise and fall
together across transects?
And 
(\textbf{3})
...
setup).
Our field sampling was slightly modified relative to the standard Reef Check 
approach to maximize the ability of the ROV and divers to survey the same 
benthos.
...
and, most recently, British Columbia and northern Baja, Mexico, with specific 
species and categories varying by geography and latitude \citep{Freiwald2021, 
Freiwald:2023aa}.
...
photographs are summarized in \figref{fig:camera_settings}.
...
Flight Controller, using the Extended Kalman Filter (EKF) available in ArduSub, the
...
This approach generated approximately 30 ROV photos per each side of the 30 $m$ 
transect tape, totaling 60 ROV photos per 30 x 2 $m$ diver transect. 
...
usable for downstream classification (i.e., data extraction): each photograph 
was imported into Adobe Lightroom Classic, cropped to remove lens distortion at 
the image margins, denoised, and adjusted for white balance, contrast, 
sharpness, and color, before being exported as a JPEG (see 
\citealt{Randell:2026aa} and its supplementary material for the complete
...
annotations around every observed instance of a visible macroinvertebrate. 
...
to roughly 25\% of a patch's area belonged to a neighboring category, 
...
Kelp Quest currently comprises two volunteer-facing workflows (plus 
internal ``Expert'' variants of each, used by our team for direct expert 
...
time: a stable multiplicative offset between platforms is simply a scaling factor,
which can in principle be calibrated away.
...
%% --------------------------------------------------------------------
\subsubsection{Abundance of discrete macroinvertebrates}
...
photos within an ROV transect to generate a single, transect-scale average. 

Once these transect-scale averages were generated, we next had to reconcile 
the differences in resolution across categories for both platforms that 
...
Three properties of this correlation are worth noting as they pertain to 
...
correlation coefficients and ellipse overlays for sites, season, and depth ---
...
overlapping macroinvertebrate taxa and the ROV recorded 1{,}206, for a
...
similar snapshot of macroinvertebrate abundance across methods, the models
...
five substrate categories using only the 12 winter transects (24 observations: 12 transects 
$\times$ 2 platforms), when annual kelp and algae cover is largely senesced 
...
We return to the hypothesized mechanism responsible for this seasonal 
...
$p=1.7\times10^{-8}$).
Because each kelp is concentrated at a single site, we also applied the
stricter within-site test described in the \textit{Methods}, which removes
each site's own mean before correlating; both correlations held
(\textit{Saccharina} $r=0.95$, $p=3.9\times10^{-12}$; \textit{Agarum}
$r=0.82$, $p=1.5\times10^{-6}$), so the agreement reflects which transects
within a site had more kelp, and not merely which site had kelp at all.
...
\textit{Methods}, leaves every vegetation category essentially
untouched --- green algae $r=0.92$ ($p=5.6\times10^{-10}$) and red algae
$r=0.76$ ($p=2.8\times10^{-5}$) within site, alongside \textit{Saccharina}
($r=0.95$) and \textit{Agarum} ($r=0.82$) reported above --- but substantially
deflates the
...
site-scale contrast.
Coastal vegetation is therefore the one group in which every category we
compared retained strong ($r \geq 0.76$), significant pattern agreement within
site; among invertebrates, significant within-site agreement held for four of
the ten taxa (excluding rock crab, whose within-site value inherits the
single-transect artifact described above), and among substrate categories
only pebble retained significant, though moderate, agreement.
...
shallow-transect ellipses overlap only slightly, while season
...
crucially, from the perspective of coastal habitat monitoring, we see strong
and significant agreement for every vegetation category we compared --- red
and green algae, \textit{Saccharina}, and \textit{Agarum} --- and this
agreement persists after between-site variation is removed.
We likewise see strong and significant agreement for most discrete invertebrate 
taxa, weak-to-absent agreement for a specific minority of categories and 
abundances (cobble, shell hash, sand, kelp crab, large anemone), and, for 
substrate in particular, agreement that operates largely at the between-site 
scale rather than the transect scale.
...
types from the ROV --- abundances of macroinvertebrates, and percent-cover of
...
macroinvertebrates (though not statistically significantly more) relative to
...
In practice however, both platforms may slightly vary in their width. 
Divers for example will likely exhibit some degree of variation, though they 
are trained to know what a $1m$ distance is on their body (e.g., from their 
finger tips to their sternum).
With the ROV at an altitude of $0.8~m$ above the seafloor, the approximate 
width of the photograph captured by the GoPro (once it has been cropped) is 
approximately $1.2~m$, though the exact image width will covary with 
fluctuations in ROV altitude throughout a survey. 
...
Additionally, roll about the vehicle's long axis would shift and skew the 
imaged swath laterally, independent of altitude, and pitch would do the same 
fore and aft. 
However we fly with a stabilized flight mode, so any roll and pitch will be 
minimized.  
We have not, however, quantified them, though future investigation could: 
roll and pitch are recorded continuously in the vehicle's telemetry logs 
alongside the altitude and position data we already use, so the diagnostic is 
available in data we have already collected and simply has not yet been 
extracted. 
\figref{fig:survey_geometry} illustrates the resulting geometry for the diver 
vs ROV snapshot, including the $\sim 0.2~m$ of extra width discussed here 
relative to the diver's swath.
...
A pair of lasers mounted downwards on the ROV is the traditional approach to 
generate scale, and a lower-cost version of these could be explored. 
...
generate size estimates for macroinvertebrates, an inference of considerable 
...
For most of the macroinvertebrate transects, density was only low/moderate: 
...
recorded 167 total macroinvertebrates ($2.78~m^{-1}$), whereas the divers 
...
our approximate survey speed of 0.1--0.13 $\frac{m}{s}$, this results in 
...
macroinvertebrates in their totality across the benthos --- virtually 
...
Third, we annotated every ``object'' (every macroinvertebrate) we could see in
...
We are uncertain what the smallest size the ROV can detect is --- detectability 
in a top-down photograph is governed less by absolute size than by contrast 
against the surrounding substrate, by morphological distinctiveness, and by 
whether the individual is exposed or partly obscured by algae: a small, 
high-contrast ochre star on sand is readily identified at a size well below 
that of a cryptic, substrate-matched individual we would not confidently assign 
to species at all.
...
Swath-width error can bias diver counts in either direction, but missed
individuals and size conservatism would both push diver counts down relative
to the ROV, in the same direction as the ROV-side effects above, so we cannot
cleanly partition the consistent ROV-high tendency between the two platforms'
respective biases.
...
the total number of proportion observations the ROV generates per transect, and as
(\textbf{2}) 
...
For the diver UPC data, the diver samples the benthos via a 
point survey at 30 fixed points (once per meter along the $30~m$ 
...
taxa, e.g., what area in units of square meters is covered by various kelp 
species? 
...
For substrate, the two platforms are not two measurements of the same quantity 
...
the winter, as we did here as part of this study --- doing so substantially reduces the 
statistically significant differences between ROV- and diver-derived 
substrate estimates.
We do not, however, have an explanation for why boulder, CCA, and shell hash
...
overlaid z-scores (\figref{fig:z-scores}), and by the fact that both
correlations held within site.
...
macrophytes.
This was additionally exemplified by our machine learning model's ability to effectively
...
and instead, we would have to settle for a slightly broader or grouped level
of classification.
...
We also note that the absence of scale is among the more tractable
...
We also find our NMDS ordination of all 1{,}436 photos from the ROV-diver
...
one another across all categories and species.
A model comparing levels asks whether the ROV records more or less of
something than divers do.
A correlation asks whether the two platforms agree about pattern, i.e., where 
and when there was more of it.
...
Crucially, for the purposes of capturing information about coastal vegetation,
we see agreement in both level and pattern for red and green algae, and strong
agreement in pattern for \textit{Saccharina} and \textit{Agarum}, whose
differing units between platforms preclude a direct comparison of level.
The alignment of these categories is the single most important and
encouraging finding when considering how ROV-derived metrics relate to diver
data, and it survived the removal of between-site variation
(\textit{Saccharina} $r=0.95$, \textit{Agarum} $r=0.82$, red algae $r=0.76$,
green algae $r=0.92$ within site).
In other words, the ROV does not merely distinguish a kelp-dominated site from
a bare one; it tracks which transects within a site support more or less
vegetation, which is the resolution at which a monitoring or restoration
program working at a single site would operate.
The alignment between level and pattern then begins to exhibit variation when 
we consider substrate.
...
exactly what the mechanism proposed earlier in this \textit{Discussion} predicts: when
...
is the appropriate approach for a report whose purpose is to evaluate a new
...
The central finding of this report is that the ROV and Reef Check divers
characterize coastal vegetation similarly.
Red and green algal percent-cover showed no significant difference in level
between platforms, and all four vegetation categories we compared --- red
algae, green algae, \textit{Saccharina}, and \textit{Agarum} --- showed strong
agreement in pattern across transects that persisted after between-site
variation was removed (within-site $r=0.76$--$0.95$).
For kelp forest conservation and restoration, where vegetation is typically
the primary target of monitoring, this is the most consequential result of
this comparison.

More broadly, across both derived metrics investigated in this report ---
abundance of macroinvertebrates and percent-cover of macroalgae and
substrate --- the ROV performed statistically indistinguishably from Reef Check divers for the
...
level and agreement in pattern are largely independent for substrate 
categories: boulder differs
...
course.
On this stricter reading, coastal vegetation is where the ROV is most clearly
ready to stand in for or alongside divers, for the reasons summarized above,
...
that it resolves substrate pattern among transects within a site in the same 
fashion as divers. 
...
We are also grateful to Alyssa Lind, Kim Thomas, Madison Churchill, and Amy Olsen 
...
providing vessel support for ROV surveys in the summer of 2024. 
...
Indianola, where the ROV is monitoring bull kelp restoration with the Puget Sound 
Restoration Fund (not part of this technical report).
The two focal sites detailed in this report are Sirens of Spring 
...
of 2024;
Row 3 depicts three images from the winter of 2025 (both depths).
Elliott Bay Marina breakwater consists of hard substrate composed of
...
\end{subfigure}
\caption{
(a) The Seattle Aquarium's 24' aluminum Almar vessel off which we conducted paired 
...
\centering
\begin{tikzpicture}
\clip (0,0) rectangle (0.56\textwidth,0.3733\textwidth);
\node[anchor=center,inner sep=0] at (0.28\textwidth,0.18667\textwidth)
{\includegraphics[width=0.56\textwidth]{Lutris_1.jpeg}};
\end{tikzpicture}
\vspace{0.005\textheight}
\begin{tikzpicture}
\clip (0,0) rectangle (0.56\textwidth,0.3733\textwidth);
\node[anchor=center,inner sep=0] at (0.28\textwidth,0.18667\textwidth)
{\includegraphics[width=0.56\textwidth]{Lutris_2.PNG}};
\end{tikzpicture}
\vspace{0.005\textheight}
\begin{tikzpicture}
\clip (0,0) rectangle (0.56\textwidth,0.3733\textwidth);
\node[anchor=center,inner sep=0] at (0.28\textwidth,0.18667\textwidth)
{\includegraphics[height=0.3733\textwidth]{Lutris_3.PNG}};
\end{tikzpicture}
\caption{
Top: ROV \textit{Lutris} in its $V4$ configuration with integrated lighting 
(see \citealt{Randell:2026aa} for details about the various hardware 
...
\vbox to \textheight{%
	...
	\begin{figure}[H]
		...
		\end{settingsbox}
		\caption{
			The camera settings used by the GoPro HERO 12 camera with GoPro 
			Labs firmware
			installed on the camera's SD card.
			These settings enabled 27.3 megapixel raw (GPR) and preview (JPG) 
			photographs to
			be taken every three seconds.
			The GoPro cameras were also time synchronized before each flight 
			via:
			\url{https://gopro.github.io/labs/control/precisiontime/} enabling 
			alignment
			with our ROV telemetry files.
		}
...
each of which consists of $224 \times 224$ pixels.
(b) A single ``subject'' --- the individual percent-cover data point we provide 
to Kelp Quest volunteers to review. 
Each subject consists of a red target that provides the point-sampling 
visual guide, a green box delineating the $224 \times 224$ patch of pixels viewed by 
...
launch (March 24, 2026) through August 10, 2026.
The 582{,}458 total classifications over this period equate to an average of
4{,}160 classifications per day.  
}
...
and the 5 substrate categories (with \texttt{season} dropped, as no seasonal 
...
Panels (d) and (e) illustrate that agreement in level and agreement in
pattern are separable: boulder sits far below the 1:1 line yet is
...
x-axis.
The two platforms' z-scores were very strongly correlated for
\textit{Saccharina} ($r=0.96$, $p=6.3\times10^{-14}$) and \textit{Agarum}
($r=0.88$, $p=1.7\times10^{-8}$), and remained so within site ($r=0.95$ and
$r=0.82$, respectively), indicating that the ROV and the divers --- despite
recording fundamentally different quantities in different units --- agree
closely in the relative pattern of kelp across transects.
...
transect 4 (shallow, top) and transect 3 (deep, bottom; two of the six
...
Marina, where the two depth strata's ellipses overlap only slightly;
...
upon double counting invertebrates is clearly density-dependent, as exemplified by 
...
below the cutoff used by Reef Check divers to count an invertebrate. 
