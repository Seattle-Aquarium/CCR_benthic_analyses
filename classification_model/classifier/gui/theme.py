...
between them when the appearance mode changes, so every colour here is a pair
and the toggle costs nothing at runtime.
...
# bundled TTF cannot be named by Tk, so this has to happen first.
...
#: Consolas is used only for the log pane, where column alignment matters more
#: than branding.
MONO = "Consolas"

# Type scale follows the hierarchy on p.24 of the guidelines: Title is Bold,
...
FONT_H1 = (FAMILY_MEDIUM, 16)
FONT_H2 = (FAMILY_SEMIBOLD, 13)
FONT_BODY = (FAMILY, 12)
...
def logo_for(mode: str) -> str | None:
    ...
    The guidelines allow White on any dark background; Mediterranean Blue is
    the primary treatment on white or light.
    """
